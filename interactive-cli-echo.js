  ...
  const { format } = await inquirer.prompt(
    {
      ...
      loop: false,
      choices: [
        { value: PCMFormat.RTAUDIO_SINT8, name: '8 bit' },
        { value: PCMFormat.RTAUDIO_SINT16, name: '16 bits' },
        { value: PCMFormat.RTAUDIO_SINT32, name: '32 bits' },
        { value: PCMFormat.RTAUDIO_FLOAT32, name: '32 bits (float)' },
        { value: PCMFormat.RTAUDIO_FLOAT64, name: '64 bits (float)' },
      ],
      default: () => 1
    },
  )
...
const main = async () => {
  const app = async () => {
    consoleClear()
    cursor.reset().write('')
    cursor.bold().write('NodeAudio ').reset().brightYellow().write(version).reset().nextLine()
    cursor.bold().write('RTAudio ').reset().brightYellow().write(`${rtAudioVersion}`).nextLine().nextLine()
    ...
    audioReadStream = createAudioReadStream({
      api: api,
    ...
    audioWriteStream = createAudioWriteStream({
      api: api,
