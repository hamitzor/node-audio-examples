...
import { createAudioReadStream, createAudioWriteStream, probeApis, probeDevices, rtAudioVersion, PCMFormat } from '@hamitzor/node-audio'
...
const audioReadStream = createAudioReadStream({
  api: api.id,
  deviceId: defaultInputDevice.id,
  channels: 1,
  sampleRate: sampleRate,
  bufferFrames: sampleRate / (1000 / frameSize),
  format: PCMFormat.RTAUDIO_SINT16,
})

// Crate an audio write stream
const audioWriteStream = createAudioWriteStream({
  api: api.id,
...
console.log(`NodeAudio ${version}`)
console.log(`RTAudio ${rtAudioVersion}\n`)
